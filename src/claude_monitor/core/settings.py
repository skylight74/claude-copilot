...
class Settings(BaseSettings):
    ...
    view: Literal["realtime", "daily", "monthly", "session", "agents"] = Field(
        default="realtime",
        description="View mode (realtime, daily, monthly, session, agents)",
    )
    ...
    tui: bool = Field(
        default=False,
        description="Use interactive Textual TUI instead of Rich display",
    )
    ...
    def validate_view(cls, v: Any) -> str:
        ...
        if isinstance(v, str):
            v_lower = v.lower()
            valid_views = ["realtime", "daily", "monthly", "session", "agents"]
