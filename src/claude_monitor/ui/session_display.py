...
from claude_monitor.ui.decision_support import format_decision_support_panel
...
class SessionDisplayComponent:
    ...
    def format_active_session_screen(
        ...
        **kwargs,
        ...
        # Calculate cost per minute for work capacity
        cost_per_min = (
        ...
        # =================================================================
        # TIER 1: DECISION SUPPORT PANEL (Always visible, at-a-glance)
        # =================================================================
        screen_buffer.append("")

        # Add decision support panel content
        decision_panel = format_decision_support_panel(
            tokens_used=tokens_used,
            token_limit=token_limit,
            session_cost=session_cost,
            cost_limit=cost_limit_p90,
            sent_messages=sent_messages,
            messages_limit=messages_limit_p90,
            elapsed_minutes=elapsed_session_minutes,
            total_minutes=total_session_minutes,
            burn_rate=burn_rate,
            cost_per_minute=cost_per_min,
        )
        screen_buffer.extend(decision_panel)

        # =================================================================
        # TIER 2: CONTEXTUAL DETAILS (Below fold)
        # =================================================================
        screen_buffer.append("")
        screen_buffer.append(f"[separator]{'─' * 70}[/]")
        screen_buffer.append("[header]📋 SESSION DETAILS[/]")
        screen_buffer.append("")
        ...
        screen_buffer.append(
            f"   [info]Tokens run out:[/]  [warning]{predicted_end_str}[/]"
        )
        screen_buffer.append(
            f"   [info]Limit resets:[/]    [success]{reset_time_str}[/]"
        )
        screen_buffer.append("")
        ...
        # Footer with session health indicator
        time_remaining = max(0, total_session_minutes - elapsed_session_minutes)
        status_key = get_session_health_status(usage_percentage, time_remaining)
        status = SESSION_HEALTH_STATUS[status_key]

        screen_buffer.append(
            f"⏰ [dim]{current_time_str}[/] 📝 [{status['border_style']}]Active session[/] | "
            f"[dim]Ctrl+C to exit[/] {status['emoji']}"
        )
