...
# Session Health Status Configuration
SESSION_HEALTH_STATUS: Dict[str, Dict[str, str]] = {
    "continue": {
        "emoji": "🟢",
        "label": "CONTINUE",
        "message": "Plenty of capacity for complex work",
        "border_style": "success",
        "title_style": "bold success",
    },
    "wrap_up": {
        "emoji": "🟡",
        "label": "WRAP UP",
        "message": "Start wrapping up current work",
    ...
    "finish": {
        "emoji": "🔴",
        "label": "FINISH NOW",
        "message": "Minimal capacity remaining",
        "border_style": "error",
        "title_style": "bold error",
    },
    "reset_soon": {
        "emoji": "🔵",
        "label": "RESET SOON",
        "message": "Limit resets within 30 minutes",
        "border_style": "info",
        "title_style": "bold info",
    },
}

# Session Health Recommendations
SESSION_RECOMMENDATIONS: Dict[str, str] = {
    "continue": "Continue working - ideal time for complex tasks",
    "wrap_up": "Wrap up - save complex work for next session",
    "finish": "FINISH NOW - save work and stop",
    "reset_soon": "Reset approaching - finish up or wait for fresh session",
}

# Efficiency Star Ratings
EFFICIENCY_STARS: Dict[float, str] = {
    5.0: "★★★★★",
    4.0: "★★★★☆",
    3.0: "★★★☆☆",
    2.0: "★★☆☆☆",
...
def get_efficiency_stars(score: float) -> str:
    """Get star rating for efficiency score (0-5.0).

    Args:
        score: Efficiency score between 0 and 5.

    Returns:
        Star rating string representation.
    """
    for threshold, stars in sorted(EFFICIENCY_STARS.items(), reverse=True):
        if score >= threshold:
            return stars
    return "☆☆☆☆☆"
...
def get_session_health_status(
    ...
    """Determine session health status based on usage and time.

    Args:
        usage_pct: Current usage percentage (0-100+).
        time_remaining_minutes: Minutes until session reset.

    Returns:
        Status key: 'continue', 'wrap_up', 'finish', or 'reset_soon'.
    """
...
def get_progress_bar_style(percentage: float) -> Dict[str, str]:
    """Get dynamic progress bar styling based on percentage.

    Args:
        percentage: Current usage percentage.

    Returns:
        Dictionary with filled_style, empty_style, emoji, and status keys.
    """
    if percentage >= 90:
        return {
            "filled_style": "error",
