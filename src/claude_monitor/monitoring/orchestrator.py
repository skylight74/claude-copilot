...
class MonitoringOrchestrator:
    ...
    def __init__(
        ...
        self._detected_token_limit: Optional[int] = None  # From auto-detection
    ...
    def set_detected_token_limit(self, token_limit: int) -> None:
        """Set the detected token limit from auto-detection.

        When set, this limit will be used instead of recalculating on each cycle.

        Args:
            token_limit: The detected token limit
        """
        self._detected_token_limit = token_limit
        logger.info(f"Using detected token limit: {token_limit:,}")
    ...
    def _calculate_token_limit(self, data: Dict[str, Any]) -> int:
        ...
        If a detected token limit was set via set_detected_token_limit(),
        that value is used instead of recalculating.
        ...
        Returns:
            Token limit
        """
        # Use detected limit if set (from auto-detection at startup)
        if self._detected_token_limit is not None:
            return self._detected_token_limit
