...
const results = [];
function logResult(testName, status, duration, error, details) {
...
function registerHook(taskId, callback, options = {}) {
...
function getTaskHooks(taskId) {
    return Array.from(hookRegistry.values()).filter(h => h.taskId === taskId);
}
function clearHooks() {
    hookRegistry.clear();
}
function parseCompletionPromises(output) {
    const promises = [];
...
function evaluateDefaultHook(context) {
...
// ============================================================================
function matchPatterns(files, skills) {
    const results = [];
...
function matchKeywords(text, skills) {
    const results = [];
...
function calculateCombinedConfidence(patternConfidence, keywordConfidence, options = { patternWeight: 0.5, keywordWeight: 0.5 }) {
...
function matchCorrectionPatterns(message) {
    const matches = [];
...
function calculateCorrectionConfidence(matches, messageLength, hasPreviousOutput) {
    if (matches.length === 0)
        return 0;
    const weights = {
...
function extractValues(matches) {
    let oldValue;
    let newValue;
    for (const match of matches) {
        if (!match.captures)
...
function inferTarget(message, agentId) {
...
async function runStreamCTests() {
    ...
    await runTest('Pattern matching with file extensions', () => {
        ...
        assertEquals(results[0].matchedPatterns.length, 2, 'Should match two files');
    });
...
async function runStreamDTests() {
    ...
    await runTest('High confidence for explicit corrections', () => {
        const confidence = calculateCorrectionConfidence([{ patternId: 'explicit-correction' }], 50, true);
    ...
    await runTest('Boost confidence for multiple patterns', () => {
        ...
        const multiplePatterns = calculateCorrectionConfidence([{ patternId: 'no-wrong' }, { patternId: 'should-be' }], 100, false);
...
async function runE2ETests() {
    ...
    await runTest('Skill-related prompt triggers skill evaluation', () => {
        ...
        const matches = Array.from(skills.values()).filter(skill => skill.keywords.some(k => userPrompt.toLowerCase().includes(k.toLowerCase())));
