...
class StatusMonitor:
    ...
    def _group_by_phase(self, streams: List[dict]) -> Dict[str, List[dict]]:
        ...
        for stream in streams:
            phase = stream.get('streamPhase', 'parallel')
            if phase in phases:
    ...
    def _render_stream(self, stream: dict, activity: List[dict]):
        ...
        stream_name = stream.get('streamName', 'unnamed')
        phase = stream.get('streamPhase', 'parallel')
        total_tasks = stream.get('totalTasks', 0)
        completed_tasks = stream.get('completedTasks', 0)
        ...
        if stream_activity and status.lower() not in ['completed', 'pending']:
            ...
            agent = latest.get('agentId', 'unknown')
            task_title = latest.get('taskTitle', 'working')
            is_active = latest.get('isActive', False)

            # Only show if actually active
            if is_active:
                # Calculate visible content for padding
                activity_plain = f"    └─ @agent-{agent}: {task_title}"
                # Truncate if too long
                if len(activity_plain) > 64:
                    task_title = task_title[:64 - len(f"    └─ @agent-{agent}: ") - 3] + "..."
                    activity_plain = f"    └─ @agent-{agent}: {task_title}"

                activity_text = self._color(f"    └─ @agent-{agent}: {task_title}", Colors.CYAN)
    ...
    def display(self):
        ...
        if streams is None:
            self._render_error_screen(
                ...
                f"  Verify HTTP_API_PORT environment variable"
            )
            return
